...
3. If no project is linked, or the linked project differs from the user's explicit target, ask for the target only when it is unclear. Run `html-publish link --project "<id, slug, or exact name>"`. The CLI selects a saved project credential or opens browser authorization automatically. Tell the user to confirm the locked target project and click Allow, then keep waiting for the same CLI process. Never ask for a token or tell the user to run `html-publish login`.
