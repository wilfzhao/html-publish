...
type Authorization = { userCode: string; clientName: string | null; status: string; expiresAt: string; project: Project | null };
...
function ConnectContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/auth/device?code=${encodeURIComponent(initialCode)}`)
      .then((res) => res.ok ? res.json() : Promise.reject(new Error('Authorization request expired or not found.')))
      .then(async (auth: Authorization) => {
      setAuthorization(auth);
      if (auth.project) {
        setProjects([auth.project]);
        setSelectedProject(auth.project.id);
        return;
      }
      const projectResponse = await fetch('/api/projects');
      if (!projectResponse.ok) throw new Error('Unable to load projects.');
      const projectList: Project[] = await projectResponse.json();
      setProjects(projectList);
      const hinted = projectList.find((project: Project) =>
        project.id.toLowerCase() === projectHint
        || project.slug.toLowerCase() === projectHint
        || project.name.toLowerCase() === projectHint
      );
  ...
  return (
    <ConnectShell>
      ...
      {authorization?.project ? (
        <div className="mt-5 rounded-xl border border-indigo-100 bg-indigo-50/60 px-4 py-4 text-left">
          <div className="text-xs font-semibold uppercase tracking-wide text-indigo-500">Target project</div>
          <div className="mt-2 flex items-center gap-3">
            <span className="text-2xl" aria-hidden="true">{authorization.project.icon || '📄'}</span>
            <div>
              <div className="font-semibold text-gray-900">{authorization.project.name}</div>
              <div className="text-xs text-gray-500">{authorization.project.slug}</div>
            </div>
          </div>
        </div>
      ) : (
